...
template <typename T> Napi::Value tensorToArray(Napi::Env env, const torch::Tensor& tensor)
{
...
Napi::Value Tensor::Slice(const Napi::CallbackInfo& info)
{
    ...
    auto sliceDims
        = info.Length() >= 1 && info[0].IsNumber() ? info[0].As<Napi::Number>().Int64Value() : 0;
    auto sliceStart
        = info.Length() >= 2 && info[1].IsNumber() ? info[1].As<Napi::Number>().Int64Value() : NULL;
    auto sliceEnd
        = info.Length() >= 3 && info[2].IsNumber() ? info[2].As<Napi::Number>().Int64Value() : NULL;
